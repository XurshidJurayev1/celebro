        ...
        ? dataOrder.map((order, id) => order.serviceType.toLowerCase().includes(name.toLowerCase()) && (
          <div key={id} className={style.tableDetail}>
            ...
            <div className={style.twoButtons}>
              {props.admin.orderPermission === 'TRUE' &&
              <Link to="/admin/delete/orders" onClick={() => props.selectedAnyItem(order)}>Delete</Link>}
              {props.admin.orderPermission === 'TRUE' &&
              <Link to="/admin/read/orders" onClick={() => props.ReadOrder(order)}> Read</Link>}
              {props.admin.orderPermission === 'TRUE' &&
              <Link to="/admin/edit/orders" onClick={() => props.selectedAnyItem(order)}> Edit</Link>}
